...
points_loop_cond:
	...
	mul	$t4, $t0, POINT2D_SZ	# offset = i * size of each array element
	la	$t5, my_points		# base address
	add	$t4, $t4, $t5 		# address of my_points[i]

	lw	$t1, ROW_OFFSET($t4)	# row in $t1
					
	lw	$t2, COL_OFFSET($t4)	# int col = my_points[i].col;
					
					# int height = topography_grid[row][col];
	mul	$t4, $t1, MAP_SIZE
	add	$t4, $t4, $t2		# row * (number of cols in each row) + col
	mul	$t4, $t4, INT_SIZE	# offset
	la	$t5, topography_grid	# base address
	add	$t4, $t5, $t4		# address of topography_grid[row][col]
